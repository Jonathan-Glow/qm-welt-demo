...
(function () {
  ...
  if (fill) {
    ...
    if (pctNum) {
      ...
      const tick = (now) => {
        ...
        pctNum.textContent = Math.round(p * target);
  ...
  /* ── Untermenü einklappen (persistent) ────── */
  const caret = document.getElementById("nav-caret");
  const sub = document.getElementById("side-sub");
  if (caret && sub) {
    const KEY = "qm-sub-collapsed";
